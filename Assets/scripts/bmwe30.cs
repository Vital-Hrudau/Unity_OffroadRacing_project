...
public class bmwe30 : MonoBehaviour {
    ...
    private void Start()
    {
        ...
        HPbmw = PlayerPrefs.GetInt("HPbmw");
        HPbmw = 1400;
        if (!PlayerPrefs.HasKey("HPbmw"))
        {
            PlayerPrefs.SetInt("HPbmw", 1250);
        }
    }
    public void Forward()
    {
    ...
    void Update()
    {
        ...
        PlayerPrefs.SetInt("HPbmw", HPbmw);
        ...
        if (horizont < -0.8 || horizont > 0.8)
        {
            foreach (WheelJoint2D joint in wheels)
            {
                ...
                motor.motorSpeed = HPbmw * horizont * -1;
                joint.motor = motor;
                joint.useMotor = true;
            }
        }
