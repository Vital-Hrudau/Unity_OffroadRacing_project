...
public class UAZControl : MonoBehaviour
{

    public WheelJoint2D[] wheels;
    public float horizont;
    public int HPuaz;
    public GameObject carbody, Uaz;
    ...
    void Start()
    {
        ...
        x = currentFuel / fuelSize;
        if (!PlayerPrefs.HasKey("HPuaz"))
        {
            PlayerPrefs.SetInt("HPuaz", 1130);
        }  
    }
    public void Forward()
    {
    ...
    public void Update()
    {
        ...
        //fuelProgressBar.GetComponent<Image>().color = new Color (2.0f *x, 2.0f * (1-x), 0);
        ...
        if (horizont < -0.8 || horizont > 0.8)
        {
            foreach (WheelJoint2D joint in wheels)
            {
                ...
                motor.motorSpeed = HPuaz * horizont * -1;
                joint.motor = motor;
                joint.useMotor = true;
            }
        }
        ...
        //nivascript.HPniva += 5f; изменение переменной мощности нивы
    }
    void FixedUpdate()
    {
        if (currentFuel <= 0)
    ...
    private void OnTriggerEnter2D(Collider2D trigger)
    {
        ...
        {
            Uaz.transform.localPosition = new Vector3(-98f, 15f);
